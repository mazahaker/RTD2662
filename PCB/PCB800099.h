// Configuration header file for the PCB800099 rtd2660/2 lcd driver board. Added some inline explanations of pin
// configuration and stuff so I know where to find it
...
// ~~~ About pin config ~~~ //
// _Pin_xy refers to physical rtd2660 pin. PxDy refers to mcu port/pin number. _MCU_PORT_xy refers to PxDy.
// ADCAx pins are 6-bit, ADCBx are 10-bit. 

// Only a few of the huge number of 2660/2 pins are easily accessible on the pcb800099. A few could potentially be repurposed 
// into doing something useful, like _PIN_52, _PIN_64 and _PIN_65.

// Also, _PIN_50 can read a DC voltage. 


// _PIN_50 is configured as the 6-bit ADCA0, and connected to the DC input jack through what looks like a voltage divider.
...
// _PIN_52 is connected through a 330 ohm resistor to the LED_R pin on the keypad header. 
#define _PIN_52 								(2 & 0x03)	// 0 ~ 3 (0: P6D2i, 1: P6D2o<open-drain>, 2: P6D2o<push-pull>, 3: ADCA2)

// _PIN_53 is connected to the resistor network connecting to the input keypad header. It's configured as the 6-bit ADCA3.
// ADC values' functions are defined in the _KEY_SCAN_TYPE thing below, and input is handled by the Key_PCB2660_003_5Key .h/.c combo.
...
//DON'T enable this or you won't be able to address the eeprom over the i2c pins on the vga header
...
// _PIN_64 is connected straight and without protection/pullups to the IR pin on the keypad header. 
#define _PIN_64 								(7 & 0x07)  // 0 ~ 7 (0: P1D0i, 1: P1D0o<open-drain>, 2: P1D0o<push-pull>, 3: T2, 4: SD0, 5: SPDIF0, 6: TCON[0], 7: IrDA)

// _PIN_65 is connected through a 330 ohm resistor to the LED_G pin on the keypad header. It's also got a 1k pullup to 3.3v.
...
// PIN_111 (P3D4) is the backlight enable pin. It's got a 472 pullup, and goes through a lower value resistor to the base of a 1am 
// transistor.The transistor's collector is connected to the enable pin of the PT4103 lcd boost driver, 
// and then to the ON/OFF pin on the 6-pin power header. 
